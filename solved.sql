...
-- BLOB data type is used for images --
ALTER TABLE actor
...
SELECT last_name AS "Last Name", COUNT(*) AS "Actors with this Name"
FROM actor
GROUP BY last_name;

-- * 4b. List last names of actors and the number of actors who have that last name, but only for names that are shared by at least two actors --
SELECT last_name, COUNT(*) AS "Actors with this Name"
FROM actor 
GROUP BY last_name
HAVING COUNT(*) >=2;
...
-- I didnt know the ID im looking for --
...
SELECT s.first_name, s.last_name, SUM(p.amount) AS "Amount Spent"
FROM staff s
...
SELECT COUNT(inventory_id) AS "Number of Hunchback Impossible Copies"
FROM film f
...
SELECT c.first_name, c.last_name, SUM(p.amount) AS "Total Amount Spent"
FROM customer c
INNER JOIN payment p
...
SELECT CONCAT(cu.first_name, " ", cu.last_name) as "Full Name", cu.email
FROM customer cu
...
SELECT s.store_id, SUM(amount) AS "Business Revenue"
FROM payment p
INNER JOIN rental r
...
GROUP BY c.name ORDER BY "Gross Revenue" ASC;
...
JOIN payment p
ON (r.rental_id = p.rental_id)
GROUP BY c.name ORDER BY "Gross Revenue" ASC LIMIT 5;
